<script>
    ...
    async function copyEmail() {
        try {
            await navigator.clipboard.writeText('raayyantariq@skiff.com');
...
{#if modalVisible}
    <div in:fade={{duration: 300, easing: cubicOut}} out:fade={{duration: 200, easing: cubicOut}} on:click={() => {modalVisible = false}} class="fixed top-0 left-0 w-full h-full backdrop-blur-lg z-10 bg-[rgba(253,249,236,0.8)] dark:bg-[rgba(20,20,12,.85)]"></div>
    ...
    class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-108 z-10">
        <div class="ios-nav-in flex flex-col items-center gap-6 w-full h-full p-2 pt-4 bg-surface-container shadow-lg rounded-2xl">
            <div class="flex items-center justify-center relative">
                <div class="rounded-full border-2 border-primary w-3 h-3 items-center justify-center flex">
                    <div class="rounded-full bg-primary w-1 h-1 items-center justify-center flex"></div>
                </div>
            ...
            <button on:click={() => {modalVisible = false}} style="scale: 0;" 
                class="scale-up animation-delay-50 easing-elastic absolute top-0 right-4 -mt-4 rounded-full button-shadow-default1 border border-[rgba(201,195,184,.5)] dark:border-outline-variant h-8 w-8 bg-surface-container-high text-outline lg:text-on-surface flex items-center justify-center md:hover:brightness-95 active:brightness-95 md:active:brightness-[.93] active:scale-95 transition-all duration-100 flex-shrink-0">
            ...
            <div class="flex gap-2 border border-outline-variant border-dashed rounded-2xl p-2 pt-16 overflow-hidden w-full">
                <div class="button-animation animation-delay-50">
                ...
                <div class="button-animation animation-delay-150" style="--button-initial-rotation: 20deg;">
                ...
                <div class="button-animation animation-delay-250" style="--button-initial-rotation: 30deg;">
...
<style lang="scss">
    ...
    .pin-animation {
        animation: pin-animation 500ms cubic-bezier(0, 0.885, 0.32, 1) forwards;
        // scale: 2;
    ...
    .ios-nav-in {
        animation: ios-nav-in 500ms cubic-bezier(0, 0.885, 0.32, 1);
